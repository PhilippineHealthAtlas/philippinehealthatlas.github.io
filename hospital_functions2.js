...
//console.log(categories);
	...
	console.log(categories);
	var newDiv = newCheckboxLine(m,togglePointCheckboxes);
	$('#addressbar .padding').append(newDiv);
	if (m.groupby) {
		var myGroup = $('<div>').addClass('indented').attr('id','cb_sub_'+m.keyword);
		for (var i in m.groupby.values) {
			var newDiv = newCheckboxLine(m,filterPointCheckboxes,i);
			myGroup.append(newDiv);
		}
		$('#addressbar .padding').append(myGroup);
	}
	//console.log('--CATEGORIES--');
	//console.log(categories);
...
function newCheckboxLine(m, togglePointCheckboxes,n) {
	
	var newInput = $('<input>')
		.attr({
		'type':'checkbox', 
		'id': 'cb_' + m.keyword + (typeof n != 'undefined' ? '_' + n : ''), 
		'name': m.keyword, 
		'value' : (typeof n != 'undefined' ? m.groupby.values[n] : m.keyword ),
		'checked':(m.hideLayer ? false : true)})
		.click(togglePointCheckboxes);
	if (typeof n != 'undefined') {
		newInput.data('field',m.groupby.field);
	}
	var newLabel = $('<label>')
		.attr({'for': 'cb_' + m.keyword + (typeof n != 'undefined' ? '_' + n : '')})
		.html(' '+(typeof n != 'undefined' ? m.groupby.values[n] : m.description));
	if (typeof n == 'undefined') {
		var newImage = $('<img>')
			.attr({'src': m.marker})
			.html(m.description);
...
function loadPointSpreadsheets() {
	var initOptions = {
		zoom: (settings && settings.initzoom ? settings.initzoom : 8),
...
function filterPointCheckboxes () {
...
//console.log('categ = ' + categ);
	...
	var filter_is_checked = $(this).attr('checked');
	var categ_is_checked = ($('#cb_'+categ).attr('checked'));
	var items = categories[categ].items;
	var filter = $(this).attr('value');
	var filterField = $(this).data('field');
	for (var j = 0; j < items.length; j++) {
		if (items[j].marker) {
			if (categ_is_checked && items[j][filterField] == filter && filter_is_checked) {
				items[j].marker.setMap(map);
			} else if (categ_is_checked && items[j][filterField] == filter && !filter_is_checked) {
...
function togglePointCheckboxes () {
	...
	var categ_is_checked = ($('#cb_'+categ).attr('checked'));
	if (categ_is_checked) {
		$('#cb_sub_'+categ +' input').prop('checked',true);
	} else {
		$('#cb_sub_'+categ +' input').prop('checked',false);
	}
	console.log(categ_is_checked);
	var items = categories[categ].items;
	for (var j = 0; j < items.length; j++) {
		if (items[j].marker) {
			if (categ_is_checked) {
...
function displayPointsNew(result) {
	...
	console.log('exiting displayPointsNew');
	console.log(categories);
}
