...
//		categories[datum.category].description +':'
	...
	for (i in attributes) {
		if (parseFloat(datum[i]) > 0) {
			text+= '<tr><th>' 
			+ (attributes[i].is_indicator ? 
				'<span style="background-color:' + attributes[i].categ(datum[i]) + ';color:' + attributes[i].categ(datum[i]) + '">__</span> '
				: '')
			+ attributes[i].description + ': </th><td>'+datum[i]+'</td></tr>';
		}
	}
		text += '</table>';
