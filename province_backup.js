...
showMarkerInfoWindow = function () {
	...
	var marker;
	marker = provinces[provname]; //selse marker = cities[provname];	
	console.log(marker);
	//alert(provname);
...
function displayFeaturesNew(result) {
	...
	for (var i = 0; i < items.length; i++) {
		...
		if (items[i].provname) {
			name = stubbify(items[i].provname)
			if (provinces[name])  {
				var prov = $(provinces[name].marker);
				//console.log(prov);
				//console.log('Entered ' + categ + ' data for ' + name);
				//console.log(items[i]);
				console.log(prov);
				...
				var prov = $(cities[name].marker);
				console.log(prov);
				prov.data('datum' + categ, items[i]);
				console.log(prov);
				//console.log('Entered city data for ' + name);
				j++;
			} 
		} else {
			errors += '\n' + i + ' | ' + (items[i].provname);
