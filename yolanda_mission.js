...
var spreadsheets = [
	...
	{
		'key' : '1_r5jqCsSQYExoti1J-tdnc49HyK5KOHT2mFS1aQjo18',
		'description' : 'DOH Team',
		'keyword' : 'doh_team',
		...
		'infoText':function(datum) {
			return ''
				+'<div style="font-size:20px;font-weight:bold">Local Medical Missions: '+datum.missioncount+'</div>'
				+(datum['teamcomposition']? '<div></div><div style="height:150px; overflow:auto;">'+datum['teamcomposition'] +'</div>': '')
			
		}
	},
	{
		'key' : '1_r5jqCsSQYExoti1J-tdnc49HyK5KOHT2mFS1aQjo18',
		'description' : 'Foreign Team',
		'keyword' : 'foreign_team',
		'marker' : 'static/hospital_.png',
		'sid' : 'odb',
		'color':'#2EFE64',
		'infoText':function(datum) {
			return ''
				+'<div style="font-size:20px;font-weight:bold">Foreign Medical Missions: '+datum.missioncount+'</div>'
				+(datum['teamcomposition']? '<div></div><div style="height:150px; overflow:auto;">'+datum['teamcomposition'] +'</div>': '')
		}		
	},
	{
		...
		'infoText':function(datum) {
			return ''
				+'<div style="font-size:20px;font-weight:bold">Other Volunteer Organization: '+datum.missioncount+'</div>'
				+(datum['teamcomposition']? '<div></div><div style="height:150px; overflow:auto;">'+datum['teamcomposition'] +'</div>': '')
...
function onLoad() {
	...
	var latlngbound = new google.maps.LatLngBounds(
		new google.maps.LatLng(10.637153,119.993324),
		new google.maps.LatLng(14.513995,125.70787)
	);
	var initOptions = {
		zoom: 7,
		streetviewControl: false,
		center : new google.maps.LatLng(10.724,123.766),
	...
	for (var i = 0; i < spreadsheets.length; i++) {
		...
		var newInput = $('<input>')
			.attr({'type':'checkbox', 'id': 'cb_' + m.keyword, 'name': m.keyword, 'checked':false})
	...
	/*
	var newInput = $('<input>')
			.attr({'type':'button', 'id': 'cb_' + m.keyword , 'name': 'service'})
			.click(clearHide);
	...
	for (var i = 0; i < items.length; i++) {
		...
		*/
		var marker = new google.maps.Circle({
			//map: map,
			center: position,
			radius: Math.sqrt(items[i].missioncount) * 3000,
			fillOpacity: 0.5,
			zIndex:5,
			strokeOpacity: 1,
			strokeWeight: 1,
		...
		//google.maps.event.addListener(marker, 'mouseover', function() { this.setZIndex(4); });
