...
function loadPolygonSpreadsheets() {
	...
	if (settings.cities) {
		for (var i in city_coords) {
			...
			google.maps.event.addListener(city_marker, 'clcik', showPolygonInfoWindow);
			...
			if (datum[i] !=null && datum[i] != '') {
				...
				+ (attributes[i].is_group ? '' : ''
					...
					+ '<td>' + (attributes[i].is_indicator ? 
						'<span onmouseout="hideHelper()" onclick="showHelper(\''+ i +'\')" style="background-color:' + attributes[i].categ(datum[i]) + ';color:' + attributes[i].categ(datum[i]) + '">__</span> '
						: '') + '</td>'
				)
				+'</tr>';
			}
